  ...
  static getBody(address: string, time: string, timezone: string): string {
    switch (true) {
      case timezone?.includes('US'):
        return `${
          isDefined(address) ? 'User Location:' + address : ''
        } time: ${this.getTime(time)}`;
      case timezone?.includes('MO'):
        return `${
          isDefined(address) ? '使用者位置:' + address : ''
        } 时间: ${this.getTime(time)}`;
      default:
        return `${
          isDefined(address) ? '발생주소:' + address : ''
        } 시간: ${this.getTime(time)}`;
    }
  }
  ...
  static getTitle(
    ...
    switch (true) {
      case timezone?.includes('US'):
        return `${
          bodystate == 1
            ? 'emergency!! Heart attack issue'
            : ` ${this.getENGStatus(arrStatus)}`
        } detected!`;
      case timezone?.includes('MO'):
        return `${
          bodystate == 1 ? '紧急状况!!' : ` ${this.getChStatus(arrStatus)}`
        }`;
      default:
        return `${
          bodystate == 1 ? '긴급!! 심장마비' : this.getStatus(arrStatus)
        } 발생!`;
    }
  }
